...
contract TestExecution_TryExecuteSingle is TestExecutionBase {
  ...
  function test_TryExecuteDelegateCall_EmitTryDelegateCallUnsuccessful() public {
    ...
    vm.expectEmit(true, true, true, true);
    emit TryDelegateCallUnsuccessful(
      execution[0].callData, abi.encodeWithSelector(Counter.CounterRevertOperation.selector)
    );
