...
interface IHandler {
  ...
  function complexReturnData(
    string memory input,
    bytes4 selector
  ) external returns (uint256, bytes memory, address, uint64, address);
}
...
contract TestModuleManager_FallbackHandler is TestModuleManagerBase {
  ...
  function test_onTokenReceived_Success() public {
    ...
    assertTrue(success);
    assertTrue(
      keccak256(data) == keccak256(bytes(hex'f23a6e6100000000000000000000000000000000000000000000000000000000'))
    );
    //ERC-1155 Batch
    (success, data) = address(BOB_ACCOUNT).call{value: 0}(hex'bc197c81');
    assertTrue(success);
    assertTrue(
      keccak256(data) == keccak256(bytes(hex'bc197c8100000000000000000000000000000000000000000000000000000000'))
    );
  }
  ...
  function test_ComplexReturnData() public {
    ...
    bytes memory data =
      abi.encodeWithSelector(MockHandler(address(0)).complexReturnData.selector, testString, testSelector);
    ...
    bytes memory expectedData =
      abi.encode(testString, HANDLER_MODULE.getName(), HANDLER_MODULE.getVersion(), testSelector);
    assertEq(keccak256(resultData), keccak256(expectedData));

    vm.prank(expectedSender);
    (timestamp, resultData, addr, chainId, sender) =
      IHandler(address(BOB_ACCOUNT)).complexReturnData(testString, testSelector);
    assertEq(timestamp, block.timestamp);
    assertEq(addr, address(HANDLER_MODULE));
    assertEq(chainId, block.chainid);
    assertEq(sender, expectedSender);
