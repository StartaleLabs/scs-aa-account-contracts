...
contract TestModuleManager_UninstallModule is TestModuleManagerBase {
  ...
  function test_ModuleUninstallation_Success() public {
    ...
    assertTrue(
      BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(newMockValidator), ''), 'Module should be installed'
    );
  }
  ...
  function test_NewModuleUninstallation_Success() public {
    ...
    assertTrue(
      BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(VALIDATOR_MODULE), ''), 'Module should be installed'
    );
  }
  ...
  function test_ExecutorModuleUninstallation_Success() public {
    ...
    bytes memory installCallData =
      abi.encodeWithSelector(IModuleManager.installModule.selector, MODULE_TYPE_EXECUTOR, address(newMockExecutor), '');
    ...
    // Prepare call data for uninstalling the module
    bytes memory callData = abi.encodeWithSelector(
      IModuleManager.uninstallModule.selector, MODULE_TYPE_EXECUTOR, address(mockExecutor), abi.encode(prev, '')
    );
  ...
  function test_RevertIf_IncorrectModuleTypeUninstallation() public {
    ...
    assertFalse(
      BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(mockValidator), ''), 'Module should not be installed'
    );
    ...
    bytes memory expectedRevertReason =
      abi.encodeWithSelector(ModuleNotInstalled.selector, MODULE_TYPE_EXECUTOR, address(VALIDATOR_MODULE));
  ...
  function test_RevertIf_UninstallingNonExistentModule() public {
    ...
    // Prepare call data for uninstalling the module
    bytes memory callData = abi.encodeWithSelector(
      IModuleManager.uninstallModule.selector, MODULE_TYPE_VALIDATOR, address(mockValidator), abi.encode(prev, '')
    );
  ...
  function test_RevertIf_UninstallingNonExistentFallbackHandler() public {
    ...
    assertFalse(
      BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_FALLBACK, address(mockHandler), customData),
      'FallbackHandler should not be installed'
    );
  }
}
