...
contract TestModuleManager_InstallModule is TestModuleManagerBase {
  ...
  function test_InstallExecutorModule_Success() public {
    bytes memory callData =
      abi.encodeWithSelector(IModuleManager.installModule.selector, MODULE_TYPE_EXECUTOR, address(EXECUTOR_MODULE), '');
  ...
  function test_RevertIf_ModuleAlreadyInstalled() public {
    ...
    bytes memory expectedRevertReason =
      abi.encodeWithSignature('LinkedList_EntryAlreadyInList(address)', address(mockValidator));
  ...
  function test_InstallModule_MultiTypeInstall() public {
    ...
    bytes memory fallbackInstallData = abi.encodePacked(
      bytes4(GENERIC_FALLBACK_SELECTOR), CALLTYPE_SINGLE, bytes1(uint8(MODULE_TYPE_FALLBACK)), fallbackConfig
    );

    bytes memory hookInstallData = abi.encodePacked(bytes1(uint8(MODULE_TYPE_HOOK)), hookConfig);

    uint256[] memory types =
      Solarray.uint256s(MODULE_TYPE_VALIDATOR, MODULE_TYPE_EXECUTOR, MODULE_TYPE_FALLBACK, MODULE_TYPE_HOOK);
    bytes[] memory initDatas =
      Solarray.bytess(validatorInstallData, executorInstallData, fallbackInstallData, hookInstallData);
    ...
    assertTrue(
      BOB_ACCOUNT.isModuleInstalled(
        MODULE_TYPE_FALLBACK, address(mockMulti), abi.encode(bytes4(GENERIC_FALLBACK_SELECTOR))
      ),
  ...
  function test_InstallFallbackHandler_WithCustomData() public {
    bytes memory customData = abi.encodePacked(bytes4(GENERIC_FALLBACK_SELECTOR), CALLTYPE_SINGLE, '0x0000');
    assertFalse(
  ...
  function test_RevertIf_ReinstallFallbackHandler() public {
    ...
    PackedUserOperation[] memory userOpsFirstInstall =
      buildPackedUserOperation(BOB, BOB_ACCOUNT, EXECTYPE_DEFAULT, executionFirstInstall, address(VALIDATOR_MODULE), 0);
