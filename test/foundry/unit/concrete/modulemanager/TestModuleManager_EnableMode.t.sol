...
import {EmittingHook} from '../../../mocks/EmittingHook.sol';
...
import {TestModuleManagerBase} from '../../../shared/TestModuleManagerBase.t.sol';
import {PackedUserOperation} from '@account-abstraction/interfaces/PackedUserOperation.sol';
import 'forge-std/Test.sol';
import {EIP712} from 'solady/utils/EIP712.sol';
import {Solarray} from 'solarray/Solarray.sol';

contract TestModuleManager_EnableMode is Test, TestModuleManagerBase {
  struct TestTemps {
    bytes32 userOpHash;
    bytes32 contents;
    address signer;
  ...
  function test_EnableMode_Success_No7739_With_PreValidationHooksInstalled() public {
    ...
    assertTrue(
      BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_HOOK, address(accountLocker), ''), 'Account locker should be installed'
    );
  ...
  function test_EnableMode_Uninitialized_7702_Account() public {
    ...
    assertEq(counter.getNumber(), counterBefore + 1, 'Counter should have been incremented after single execution');
    assertTrue(
      IModuleManager(BOB_ADDRESS).isModuleInstalled(MODULE_TYPE_VALIDATOR, address(mockMultiModule), ''),
      'Module should be installed as validator'
    );
    assertTrue(
      IModuleManager(BOB_ADDRESS).isModuleInstalled(MODULE_TYPE_EXECUTOR, address(mockMultiModule), ''),
      'Module should be installed as executor'
    );
  }
  ...
  function test_EnableMode_Success_7739_Nested_712() public {
    ...
    bytes memory enableModeSig =
      abi.encodePacked(t.r, t.s, t.v, appDomainSeparator, structHash, contentsType, uint16(contentsType.length)); //prepare 7739 sig
  ...
  function test_EnableMode_Success_DeployAccount() public {
    ...
    StartaleSmartAccount EVE_ACCOUNT = StartaleSmartAccount(accountAddress);
  ...
  function test_EnableMode_FailsWhenModuleAlreadyInstalled() public {
    ...
    bytes memory revertReason =
      abi.encodeWithSignature('LinkedList_EntryAlreadyInList(address)', address(mockMultiModule));
    ...
    bytes memory expectedRevertReason =
      abi.encodeWithSelector(FailedOpWithRevert.selector, 0, 'AA23 reverted', revertReason);
  ...
  function test_EnableMode_FailsWithWrongModuleType() public {
    ...
    (bytes memory multiInstallData, bytes32 hashToSign,) =
      makeInstallDataAndHash(address(BOB_ACCOUNT), MODULE_TYPE_EXECUTOR, userOpHash); // Use EXECUTOR type instead of MULTI
  ...
  // Remediation check
  function test_Installing_Module_Recognized_ByHook() public {
    // install the emitting hook
    EmittingHook emittingHook = new EmittingHook();
    vm.startPrank(address(BOB_ACCOUNT));
    BOB_ACCOUNT.installModule(MODULE_TYPE_HOOK, address(emittingHook), '');
    vm.stopPrank();

    // prepare the module enable mode userop
    address moduleToEnable = address(mockMultiModule);
    address opValidator = address(mockMultiModule);
    PackedUserOperation memory op = makeDraftOp(opValidator);
    bytes32 userOpHash = ENTRYPOINT.getUserOpHash(op);
    ...
    // send userOp and make sure the expected selector is emitted
    vm.expectEmit(true, false, false, false, address(emittingHook));
    emit EmittingHook.PreCheckExtractedSelector(IModuleManager.installModule.selector);
    ENTRYPOINT.handleOps(userOps, payable(BOB.addr));
  }
  ...
  function makeInstallDataAndHash(
    address account,
    uint256 moduleType,
    bytes32 userOpHash
  ) internal view returns (bytes memory multiInstallData, bytes32 eip712Hash, bytes32 structHash) {
    ...
    multiInstallData = abi.encode(types, initDatas);
