...
import {EMERGENCY_UNINSTALL_TYPE_HASH} from '../../../../../src/types/Constants.sol';
import {EmergencyUninstall} from '../../../../../src/types/Structs.sol';
...
/// Review using this vs simple validator
//import { MockValidator } from "../../../mocks/MockValidator.sol";
import {MockSimpleValidator} from '../../../mocks/MockSimpleValidator.sol';

/// @title TestHook_EmergencyUninstall
/// @notice Tests for handling hooks emergency uninstall
/// todo: try the same tests with ECDSA validator.
contract TestHook_EmergencyUninstall is TestModuleManagerBase {
  ...
  function setUp() public {
    ...
    bytes memory callData = abi.encodeWithSelector(
      IModuleManager.installModule.selector, MODULE_TYPE_VALIDATOR, address(SIMPLE_VALIDATOR_MODULE), validatorSetupData
    );
    ...
    assertTrue(
      SIMPLE_VALIDATOR_MODULE.smartAccountOwners(address(BOB_ACCOUNT)) == BOB_ADDRESS, 'Bob should be the owner'
    );
  }
  ...
  function test_EmergencyUninstallHook_Initiate_Success() public {
    ...
    bytes memory callData =
      abi.encodeWithSelector(IModuleManager.installModule.selector, MODULE_TYPE_HOOK, address(HOOK_MODULE), '');
    ...
    bytes memory emergencyUninstallCalldata = abi.encodeWithSelector(
      StartaleSmartAccount.emergencyUninstallHook.selector,
    ...
    userOps[0] = buildPackedUserOp(
      address(BOB_ACCOUNT), getNonce(address(BOB_ACCOUNT), MODE_VALIDATION, address(SIMPLE_VALIDATOR_MODULE), bytes3(0))
    );
  ...
  function test_EmergencyUninstallHook_Fail_AfterInitiated() public {
    ...
    EmergencyUninstall memory emergencyUninstall =
      EmergencyUninstall({hook: address(HOOK_MODULE), hookType: MODULE_TYPE_HOOK, deInitData: '', nonce: 0});
    ...
    bytes memory emergencyUninstallCalldata =
      abi.encodeWithSelector(StartaleSmartAccount.emergencyUninstallHook.selector, emergencyUninstall, signature);
