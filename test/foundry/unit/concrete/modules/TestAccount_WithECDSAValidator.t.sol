...
import {MockSafe1271Caller} from '../../../mocks/MockSafe1271Caller.sol';
import '../../../utils/TestBase.sol';

/// @title TestECDSAValidator
/// @notice Unit tests for the ECDSAValidator contract
contract TestAccountWithECDSAValidator is TestBase {
  ...
  function setUp() public {
    ...
    // Deploy a new ECDSAValidator instance
    ...
    bytes memory callData1 = abi.encodeWithSelector(
      IModuleManager.installModule.selector, MODULE_TYPE_VALIDATOR, address(validator), k1ValidatorSetupData
    );
    bytes memory callData2 = abi.encodeWithSelector(
      IModuleManager.installModule.selector, MODULE_TYPE_VALIDATOR, address(mockSafe1271Caller), ''
    );
  ...
  function test_SetUpState() public {
    assertTrue(
      BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(validator), 'Module should be installed in setUp')
    );
  }
  ...
  function test_OnUninstall_Success() public {
    ...
    bytes memory callData = abi.encodeWithSelector(
      ...
      address(validator),
      abi.encode(prev, k1OnUninstallData)
    );
  ...
  function test_IsValidSignatureWithSender_Failure() public {
    ...
    validator.isValidSignatureWithSender(
      address(BOB_ACCOUNT), userOpHash, abi.encodePacked(signMessage(BOB, keccak256(abi.encodePacked('invalid'))))
    );
  }
  ...
  function test_Name() public {
    ...
    assertEq(contractName, 'ECDSAValidator', "Contract name should be 'ECDSAValidator'");
  }
  ...
  function test_addSafeSender_Success() public {
    ...
    validator.addSafeSender(address(mockSafe1271Caller));
    assertTrue(
      validator.isSafeSender(address(mockSafe1271Caller), address(BOB_ACCOUNT)),
      'MockSafe1271Caller should be in the safe senders list'
    );
  }
  ...
  function test_removeSafeSender_Success() public {
    ...
    assertFalse(
      validator.isSafeSender(address(mockSafe1271Caller), address(BOB_ACCOUNT)),
      'MockSafe1271Caller should be removed from the safe senders list'
    );
  }
