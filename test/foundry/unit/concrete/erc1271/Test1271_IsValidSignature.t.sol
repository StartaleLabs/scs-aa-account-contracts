...
contract TestERC1271Account_IsValidSignature is TestBase {
  struct TestTemps {
    bytes32 userOpHash;
    bytes32 contents;
    address signer;
  ...
  ERC7739Validator private validator;
  ...
  function setUp() public {
    ...
    bytes memory callData = abi.encodeWithSelector(
      IModuleManager.installModule.selector, MODULE_TYPE_VALIDATOR, address(validator), abi.encodePacked(ALICE_ADDRESS)
    );
  ...
  function test_isValidSignature_ERC6492Unwrapping() public {
    ...
    bytes4 ret =
      ALICE_ACCOUNT.isValidSignature(toContentsHash(t.contents), abi.encodePacked(address(validator), wrappedSignature));
    assertEq(ret, bytes4(0x1626ba7e));
  }
  ...
  function test_ERC7739SupportDetectionRequest() public {
    MockValidatorERC7739 validator_7739v2 = new MockValidatorERC7739();
    ...
    assertEq(
      ALICE_ACCOUNT.isValidSignature(0x7739773977397739773977397739773977397739773977397739773977397739, ''),
    ...
    assembly {
      if mload(byteChoices) {
        ...
        for { let i := 0 } lt(i, resultLength) { i := add(i, 1) } {
          mstore(0x20, gas())
          mstore8(
            add(add(result, 0x20), i), mload(add(add(byteChoices, 1), mod(keccak256(0x00, 0x40), mload(byteChoices))))
          )
        }
      }
    }
