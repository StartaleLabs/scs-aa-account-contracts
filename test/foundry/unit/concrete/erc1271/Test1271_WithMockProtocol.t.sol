...
import {TokenWithPermit} from '../../../mocks/TokenWithPermit.sol';
...
contract TestERC1271Account_MockProtocol is TestBase {
  ...
  function installK1Validator(StartaleSmartAccount account, Vm.Wallet memory user) internal {
    ...
    assertTrue(
      account.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(validator), ''), 'Validator module should be installed'
    );
  }

  function installPrevalidationHook(StartaleSmartAccount account, Vm.Wallet memory user) internal {
    // Prepare call data for installing the validator module
    bytes memory callData = abi.encodeWithSelector(
    ...
    // Assert that the validator module is installed
    assertTrue(
      account.isModuleInstalled(MODULE_TYPE_PREVALIDATION_HOOK_ERC1271, address(preValidationHook), ''),
      'PreValidationHook module should be installed'
    );
  }
}
