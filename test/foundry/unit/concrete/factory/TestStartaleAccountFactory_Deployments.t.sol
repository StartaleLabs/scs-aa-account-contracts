...
/// @title TestStartaleAccountFactory_Deployments
/// @notice Tests for deploying accounts using the StartaleAccountFactory.
contract TestStartaleAccountFactory_Deployments is TestBase {
  ...
  function test_Constructor_RevertIf_ImplementationIsZero() public {
    ...
    // Try deploying the StartaleAccountFactory with an implementation address of zero
    new StartaleAccountFactory(zeroAddress, address(this));
  }
  ...
  function test_DeployAccount_CannotReinitialize() public {
    ...
    vm.expectRevert(AccountInitializationFailed.selector);
    IStartaleSmartAccount(firstAccountAddress).initializeAccount(_initData);
  }
  ...
  function test_DeployAccount_InvalidValidatorModule() public {
    ...
    bytes memory _initDataInvalidModule =
      abi.encode(address(BOOTSTRAPPER), abi.encodeCall(BOOTSTRAPPER.initScoped, (validatorsInvalid, hook)));
  ...
  /// @notice Tests that the StartaleSmarAccount constructor reverts if the entry point address is zero.
  function test_Constructor_RevertIf_EntryPointIsZero() public {
    ...
    // Try deploying the StartaleSmartAccount contract with an entry point address of zero
    new StartaleSmartAccount(zeroAddress, address(DEFAULT_VALIDATOR_MODULE), abi.encodePacked(address(0xeEeEeEeE)));
  }
  ...
  /// @notice Tests initScoped function in Bootstrap and uses it to deploy an account with a hook module.
  function test_initScoped_WithHook_DeployAccount() public {
    BootstrapConfig[] memory validators = BootstrapLib.createArrayConfig(address(VALIDATOR_MODULE), initData);
              ...
              abi.encodePacked(
                ...
                keccak256(
                  abi.encodePacked(
                    ...
                    abi.encode(
                      FACTORY.ACCOUNT_IMPLEMENTATION(),
  ...
  function _getInitData(
    ...
    return abi.encode(address(BOOTSTRAPPER), abi.encodeCall(BOOTSTRAPPER.initScoped, (validators, hook)));
  }
}
