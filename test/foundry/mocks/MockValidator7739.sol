...
contract MockValidatorERC7739 is ERC7739Validator {
  ...
  function _validateSignatureForOwner(
    ...
    if (
      SignatureCheckerLib.isValidSignatureNowCalldata(owner, MessageHashUtils.toEthSignedMessageHash(hash), signature)
    ) {
      return true;
    }
    return false;
