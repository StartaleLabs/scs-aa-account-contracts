...
contract MockPreValidationHookMultiplexer is IPreValidationHookERC1271, IPreValidationHookERC4337 {
  ...
  error HookAlreadyInitialized(uint256 hookType);
  error HookNotInitialized(uint256 hookType);
  ...
  function onInstall(bytes calldata data) external {
    ...
    if (accountConfig[moduleType][msg.sender].initialized) {
      revert HookAlreadyInitialized(moduleType);
    }
  ...
  function preValidationHookERC4337(
    ...
    if (!config.initialized) {
      revert HookNotInitialized(MODULE_TYPE_PREVALIDATION_HOOK_ERC4337);
    }
  ...
  function preValidationHookERC1271(
    ...
    if (!config.initialized) {
      revert HookNotInitialized(MODULE_TYPE_PREVALIDATION_HOOK_ERC1271);
    }
  ...
  function isValidModuleType(uint256 moduleTypeId) internal pure returns (bool) {
    return
      moduleTypeId == MODULE_TYPE_PREVALIDATION_HOOK_ERC4337 || moduleTypeId == MODULE_TYPE_PREVALIDATION_HOOK_ERC1271;
  }
}
